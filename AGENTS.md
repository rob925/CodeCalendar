...
- `pages/api/news.js` — серверный API live-новостей для `physics` и `math`.
- `pages/_app.jsx` — подключает глобальный `styles.css`.
- `pages/_document.jsx` — базовый HTML-документ Next.js и подключение Google Fonts.
- `lib/live-news.js` — fetch/parse/map arXiv Atom и NASA RSS в формат `newsItems`.
...
- `playwright.config.js` — e2e-конфигурация Playwright с автозапуском Next dev server.
- `.github/workflows/ci.yml` — GitHub Actions проверки для `push`, Pull Request и ручного запуска.
...
- GitHub Actions запускает CI-проверки, но не деплоит GitHub Pages.
- Vercel env:
...
- fallback: `newsItemsBySubject.physics` и `newsItemsBySubject.math`;
- live: `/api/news?subject=physics|math`;
- серверный API берёт arXiv Atom для физики/математики и NASA RSS для физики;
- клиент обновляет live-новости при открытии/переключении предмета и каждые 5 часов;
- удачные live-подборки объединяются с новыми результатами и сохраняются в `localStorage` на 14 дней, чтобы оставаться между сессиями;
...
- `refreshNews()` — обновляет IT live-новости через Hacker News, physics/math через `/api/news`, или ставит fallback.
...
- Fallback-новость: добавь запись в `newsItemsBySubject[subject]`, затем `translations.ru.news[id]` и `translations.en.news[id]`.
- Live-новость: правь `lib/live-news.js` и API-источники, а клиентский кэш проверяй через `refreshNews()`.
...
- `.vercel/`, `playwright-report/` и `test-results/` не коммитить.
- Если Playwright browsers не установлены, выполни `npx playwright install chromium`.
- Если Playwright недоступен, минимум запускай `npm test`, `node --check app.js`, `node --check data/calendar-data.js`, `npm run build` и проверки данных.
